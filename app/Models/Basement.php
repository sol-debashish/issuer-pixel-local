...
class Basement extends BaseCategoryModel
{
    protected $fillable = [
        'sub_floor_id',
        'code',
    ...
    protected static $logName = 'basement_node';
    protected $nodeType = 'basement';


    public function subBasementSelectedField()
    {
        return $this->subBasement()->select('id', 'basement_id', 'code', 'industry_code', 'raw_code',  'description','note', 'status', 'cross_indexed');
    }

    public function subBasement()
    {
        return $this->hasMany(SubBasement::class) ;
    }

    public function mine()
    {
        return $this->hasMany(Mine::class, 'basement_code', 'code');
    }

    public function subMine()
    {
        return $this->hasMany(SubMine::class, 'basement_code', 'code');
    }
    ...
    {
        return $this->belongsTo(SubFloor::class);
    }
    ...
    public function getNodeSearchData($request, $data, $params = [])
    {
        ...
        if(isset($params['sub_basement_parent'])){
            $query->where('id', '<>', $params['sub_basement_parent']);
        }
        ...
        if(count($queryResult) > 0)
        {
            foreach ($queryResult as $result)
            {
                ...
                $data[$i]['id'] = "$result->code|Basement";
                $i++;
            }
        }
        return $data;
