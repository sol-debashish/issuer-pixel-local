...
class Floor extends BaseCategoryModel
{
    protected $fillable = [
        'sub_deck_id',
        'code',
    ...
    protected static $logName = 'floor_node';
    protected $nodeType = 'floor';
    ...
    public function getNodeSearchData($request, $data, $params = [])
    {
        ...
        if(isset($params['floor_code'])){
            $query->where('code', '<>', $params['floor_code']);
        }
        if(isset($params['sub_floor_parent'])){
            $query->where('id', '<>', $params['sub_floor_parent']);
        }
    ...
    public function subFloorSelectedField()
    {
        return $this->subFloor()->select('id', 'floor_id', 'code', 'industry_code',  'raw_code',  'description','note', 'status', 'cross_indexed');
    }
    ...
    {
        return $this->hasMany(Basement::class, 'floor_code', 'code');
    }

    public function subBasement()
    {
        return $this->hasMany(SubBasement::class, 'floor_code', 'code');
    }

    public function mine()
    {
        return $this->hasMany(Mine::class, 'floor_code', 'code');
    }

    public function subMine()
    {
        return $this->hasMany(SubMine::class, 'floor_code', 'code');
    }
    ...
    {
        return $this->belongsTo(SubDeck::class);
    }
