...
class SubDeck extends BaseCategoryModel
{
    protected $fillable = [
        'deck_id',
        'code',
    ...
    protected static $logName = 'sub_deck_node';
    protected $nodeType = 'sub deck';
    ...
    public function getNodeSearchData($request, $data, $params = [])
    {
        ...
        if(isset($params['sub_deck_code'])){
            $query->where('code', '<>', $params['sub_deck_code']);
        }
        if(isset($params['floor_parent'])){
            $query->where('id', '<>', $params['floor_parent']);
        }
    ...
    public function floorSelectedField()
    {
        return $this->floor()->select('id', 'sub_deck_id', 'code', 'industry_code', 'raw_code',  'description','note', 'status', 'cross_indexed');
    }
    ...
    {
        return $this->hasMany(SubFloor::class, 'sub_deck_code', 'code');
    }

    public function basement()
    {
        return $this->hasMany(Basement::class, 'sub_deck_code', 'code');
    }

    public function subBasement()
    {
        return $this->hasMany(SubBasement::class, 'sub_deck_code', 'code');
    }

    public function mine()
    {
        return $this->hasMany(Mine::class, 'sub_deck_code', 'code');
    }

    public function subMine()
    {
        return $this->hasMany(SubMine::class, 'sub_deck_code', 'code');
    }
