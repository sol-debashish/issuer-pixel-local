...
class SubEchelon extends BaseCategoryModel
{
    protected $fillable = [
        'echelon_id',
        'code',
    ...
    protected static $logName = 'sub_echelon_node';
    protected $nodeType = 'sub echelon';

    public function tierSelectedField()
    {
        return $this->tier()->select('id', 'sub_echelon_id', 'code', 'industry_code', 'raw_code',  'description','note', 'status', 'cross_indexed');
    }
    ...
    {
        return $this->hasMany(SubTier::class, 'sub_echelon_code', 'code');
    }

    public function layer()
    {
        return $this->hasMany(Layer::class, 'sub_echelon_code', 'code');
    }

    public function subLayer()
    {
        return $this->hasMany(SubLayer::class, 'sub_echelon_code', 'code');
    }

    public function deck()
    {
        return $this->hasMany(Deck::class, 'sub_echelon_code', 'code');
    }

    public function subDeck()
    {
        return $this->hasMany(SubDeck::class, 'sub_echelon_code', 'code');
    }

    public function floor()
    {
        return $this->hasMany(Floor::class, 'sub_echelon_code', 'code');
    }

    public function subFloor()
    {
        return $this->hasMany(SubFloor::class, 'sub_echelon_code', 'code');
    }

    public function basement()
    {
        return $this->hasMany(Basement::class, 'sub_echelon_code', 'code');
    }

    public function subBasement()
    {
        return $this->hasMany(SubBasement::class, 'sub_echelon_code', 'code');
    }

    public function mine()
    {
        return $this->hasMany(Mine::class, 'sub_echelon_code', 'code');
    }

    public function subMine()
    {
        return $this->hasMany(SubMine::class, 'sub_echelon_code', 'code');
    }
    ...
    public function getNodeSearchData($request, $data, $params = [])
    {
        ...
        if(isset($params['sub_echelon_code'])){
            $query->where('code', '<>', $params['sub_echelon_code']);
        }
        if(isset($params['tier_parent'])){
            $query->where('id', '<>', $params['tier_parent']);
        }
        ...
        if(count($queryResult) > 0)
        {
            foreach ($queryResult as $result)
            {
                ...
                $data[$i]['id'] = "$result->code|SubEchelon";
                $i++;
            }
        }
        return $data;
