...
namespace App\Services\ChunkUpload\Storage;
...
class ChunkStorage
{
     ...
     */
    public function __construct(FilesystemContract $disk, AbstractConfig $config)
    {
        // save the config
        $this->config = $config;
    ...
    {
        return $this->config->chunksStorageDirectory().'/'. auth()->id().'/';
    }
