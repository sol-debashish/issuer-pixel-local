...
class CreateSubMinesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('sub_mines', function (Blueprint $table) {
            $table->id();
            $table->bigInteger('mine_id')->unsigned();
            ...
            $table->string('sub_basement_code', 128)->nullable();
            $table->string('mine_code', 128)->nullable();
            ...
            $table->index(['sub_basement_code','mine_code']);

            $table->foreign('mine_id')->references('id')->on('mines')->onDelete('cascade')->onUpdate('cascade');
